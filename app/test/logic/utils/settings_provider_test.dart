...
void main() {
  ...
  group('SettingsProvider Unit Tests', () {
    ...
    test('SettingsNotifier loads correct initial state from SharedPreferences', () async {
    ...
    test('SettingsNotifier state changes update SharedPreferences and state', () async {
      final prefs = await SharedPreferences.getInstance();
      final notifier = SettingsNotifier(prefs);
    ...
    test('appAiServiceProvider instantiates CloudAiService with correct config', () async {
      final prefs = await SharedPreferences.getInstance();
      ...
      expect(cloudService.baseUrl, equals('https://generativelanguage.googleapis.com/v1beta/openai'));
    });
  });
}
